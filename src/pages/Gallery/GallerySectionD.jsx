...
export default function GallerySectionD() {
  ...
  useEffect(() => {
    ...
    const fetchGalleryData = async () => {
      try {
        ...
        setGalleryData(response.data); // Assuming response.data is an array with `title` and `images`
        setLoading(false);
      } catch (err) {
  ...
  return (
    <div className="flex flex-col items-center">
      {galleryData.map((gallery, index) => (
        <div key={index} className="container-sm flex flex-col items-start gap-[30px] md:px-5">
          <Heading
            size="text5x1"
            as="h2"
          ...
          <div className="flex flex-col items-center gap-[42px] self-stretch">
            <div className="mx-auto flex w-full gap-5 self-stretch md:mx-0 md:flex-col">
              <Slider
                ...
                ref={sliderRef}
                items={gallery.images.map((imageUrl, imgIndex) => (
            ...
            <div className="flex items-center justify-center">
              {[...Array(Math.ceil(gallery.images.length / (sliderRef?.current?.state?.itemsInSlide || 1)))].map((_, i) => (
                <div
                  key={i}
                  onClick={() =>
